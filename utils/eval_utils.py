...
def get_cls_pts_from_hm_bingdou(det_pts, cls_hm: np.ndarray):
    '''
    从热图中生成中心点
    :param hm:
    :param prob:
    ...
    pts_cls = np.zeros([len(det_pts)], np.int32)
    pts_cls_probs = np.zeros([len(det_pts), cls_hm.shape[2]], np.float32)
    hw = cls_hm.shape[:2]
    for i, pt in enumerate(det_pts):
        ...
        cls = np.argmax(probs)
        pts_cls[i] = cls
        pts_cls_probs[i] = (probs / np.sum(probs))

    return pts_cls.tolist(),pts_cls_probs
...
def draw_hm_circle(ori_im: np.ndarray, pred_pts: np.ndarray, label_pts: np.ndarray, close_dist=6,fine=False):
    '''
    绘制检测图。
    预测点绘制圆环，检测到为黄，否则为红
    标签点绘制原点，检测到为黄，否则为绿
    ...
    new_label_pts = label_pts.copy()
    pred_b = np.zeros(len(pred_pts), dtype=np.bool)
    label_b = np.zeros(len(new_label_pts), dtype=np.bool)

    for p_id, p_pt in enumerate(pred_pts):
        if len(new_label_pts) != 0:
            dists = np.linalg.norm(p_pt[None,] - new_label_pts, axis=1)
    ...
    for b, pt in zip(label_b, new_label_pts):
        if b:
        ...
        # draw_im = cv2.circle(draw_im, tuple(pt), 3, color, -1)
        # im2 = cv2.circle(im2, tuple(pt), 3, color, -1)
...
def class_map_to_contours(cm, cls_list, cls_score_thresh, use_post_pro):
    ...
    # 只查找外轮廓，并且保留少于3个点的轮廓
    all_contours = contour_tool.find_contours(has_class_bm, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE, keep_invalid_contour=True)
...
def calc_a_sample_info(out_cm, label_cm, cls_list, cls_score_thresh: Union[float, Iterable], match_iou_thresh=0.3, use_post_pro=False, ignore_board=0., *, direct_input=False, func=1):
    ...
    out_contours_found_count = np.zeros([len(out_contours)], np.int32)
    ...
    for i, (lc, lclss) in enumerate(zip(label_contours, label_clss)):
        ...
        pass_out_ids = np.argwhere(ious >= match_iou_thresh)[:, 0].astype(np.int32)
    ...
    d['fake_found_ids_in_out'].extend(np.argwhere(fake_found)[:, 0].astype(np.int32))
    return d


def calc_a_sample_info_points_each_class(pred_pm, label_pm, cls_list, match_distance_thresh_list=(5, 7, 9, 11), use_post_pro=False, use_single_pair=False, other_data=False):
    '''
    ...
    pred_cls = np.int3232(pred_cls).reshape([-1])
    label_cls = np.int3232(label_cls).reshape([-1])

    ### 我加的
    if other_data:
        pred_cls[pred_cls == 4] = 2
        pred_cls[pred_cls == 3] = 2
...
def calc_metric(pred_im, label_pos):
    ...
    pred_pos = centers
    label_found_count = np.zeros(len(label_pos), np.int32)
    pred_found_count = np.zeros(len(pred_pos), np.int32)
    for p_i, p_pos in enumerate(pred_pos):
        ...
        pred_found_count[p_i] += t.sum(dtype=np.int3232)
